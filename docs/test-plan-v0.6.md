...
## v0.6.3 Final Layout Tuning PC確認

Result: PASS

- 320 / 375 / 390 / 430px幅で横スクロールなし: PASS
- ホーム人物の見た目上の高さ比率: 男性 / 女性 1.074〜1.079: PASS
- Day人物の見た目上の高さ比率: 男性 / 女性 1.075〜1.077: PASS
- ホームの地球を20px上へ移動し、実画と街並みの空きを確保: PASS
- ホームVisual Footerの街並みを20px下へ移動: PASS
- ホームFooterの飛行機、雲、スタンプと街並みの重なりなし: PASS
- Day主要操作が320 / 375 / 390 / 430pxで最初の表示範囲内: PASS
- Day案内フッターは通常フローのflex配置で、fixed/stickyなし: PASS
- 390x844で取得前後の取得ボタン、ルート、記録、フッター、人物、街並みのY位置差: 0px: PASS
- 取得完了Toast `スタンプを取得しました`: PASS
- Toast約2秒後非表示: PASS
- NEXT自動更新、進捗更新: PASS
- 手動NEXT: PASS
- Day7 22件、Day8 21件の末尾までスクロールし、`＋記録` 残留なし: PASS
- `＋記録` 保存Toast、振り返り表示: PASS
- 再読み込み後のDayと進捗復元: PASS
- 正式157件、Day別件数、ID重複なし、巡回順: PASS
- Google Maps URL 156件、Afghanistan `地図要確認`: PASS
- `dataVersion: "1.0"`、保存、JSON Backup / Restore互換: PASS

### v0.6.3 iPhone Safari確認項目

- 390px前後で男性が女性より少し高く、両者が地球を向いている
- ホームの地球が街並みから離れて浮いて見える
- タイトル、人物、地球、吹き出し、雲、飛行機、街並みが不自然に重ならない
- ホーム下部の街並みが下寄りになり、上側の飛行機、雲、スタンプ、経路と分離している
- Day画面の主要操作が従来どおり最初の画面内で確認できる
- Day案内フッターが画面下部に自然に収まり、大きな空白や固定追従がない
- `スタンプを取得した` のタップ前後でボタン、今日のルート、`＋記録`、人物、街並みが上下に動かない
- `スタンプを取得しました` Toastが表示され、約2秒後に消える
- 取得後に進捗とNEXTが自動更新される
- Day7 / Day8の末尾までスクロールしても `＋記録` が残留しない
